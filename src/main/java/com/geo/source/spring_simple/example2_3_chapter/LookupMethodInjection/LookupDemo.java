...
public class LookupDemo {
  ...
  @Configuration
  @ComponentScan("com.geo.source.spring_simple.example2_3_chapter.LookupMethodInjection")
  ...
  private static void displayInfo(String s, DemoBean lookupBean) {
    ...
    // 每次获取都是相同的，Scope的值默认为单例（singleton）
    System.out.println(s + ": Singer Instance the Same?" + (singer1 == singer2));
    System.out.println(s + ": Singer的两个实例的值是否相等" + (singer1.equals(singer2)));
  ...
  private static void displayInfo2(String s, DemoBean2 lookupBean2) {
    ...
    // 每次获取都是都是一个新的实例，Scope的值指定为原型（prototype）
    System.out.println(s + ": Singer Instance the Same?" + (singer21 == singer22));
    System.out.println(s + ": 两个实例的值是否相等" + (singer21.equals(singer22)));
    StopWatch stopWatch = new StopWatch();
    stopWatch.start("lookupDemo");
    for (int i = 0; i < 100000; i++) {
