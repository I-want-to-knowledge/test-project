...
public class SimpleConsumer {
	...
	public static void main(String[] args) {
		...
		try {
			...
			new Thread(() -> consumer1("192.168.10.45:9092,192.168.10.69:9092,192.168.10.101:9092,192.168.10.102:9092", "topic_group1")).start();
		...
		try {
			while(true) {
				...
				records.forEach(record -> {
					...
					System.out.printf("offset = %d, key = %s, value = %s, partition = %d, value = %s, topic = %s	\n",
							record.offset(), record.key(), record.value(), record.partition(), record.value(), record.topic());
				});
			}
		} catch(Exception e) {
			e.printStackTrace();
